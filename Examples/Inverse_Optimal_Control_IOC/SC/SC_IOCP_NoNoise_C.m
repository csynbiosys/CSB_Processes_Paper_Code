...
'dNADHc = (fmax(0.5-NADH,0))^2',...                  % This state is added to handle path constraint on NADH
'dATPc = (fmax(0.7-ATP,0))^2');                      % This state is added to handle path constraint on ATP
...
%==================================
data=load('strreport_product_noiseless5.mat');    % Reads data from a .mat file
close all;
...
inputs.nlpsol.eSS.maxtime = 300;           % Maximum CPU time --- Increase if you want refined solutions
inputs.nlpsol.eSS.local.nl2sol.display = 1;
inputs.nlpsol.eSS.local.nl2sol.maxiter             =     60;        % max number of iteration
