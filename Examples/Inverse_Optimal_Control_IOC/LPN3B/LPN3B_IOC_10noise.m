...
data=load('SData_single_exp_par1_tf4.3_3pwc_SData6\strreport_single_exp_par1_tf4.3_3pwc_SData6_corrected.mat');                         % Reads data from a .mat file
...
for iexp=1:inputs.exps.n_exp 
for iexp=1:inputs.exps.n_exp
inputs.exps.exp_data{iexp}=data.newdata{1,iexp};     %postprocessed hetero_lin with errorbars not going below zero 
...
inputs.model.beta=0.01;    % Paremeter value to handle regularization in PE (any real number)
                          % 0: means no regularization   
...
inputs.nlpsol.eSS.maxtime = 100;           % Extend duration for refined solutions
