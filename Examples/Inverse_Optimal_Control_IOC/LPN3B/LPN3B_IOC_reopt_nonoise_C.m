...
inputs.pathd.runident='test_reopt';              % Run identifieer
...
inputs.model.eqns=...                                         % Equations describing system dynamics. Time derivatives are regarded 'd'st_name''
                ...
                'dS4 = k3*S3*e3',...
                'dOBJ = S2+S3',...
                'dEtot = (fmax(e1+e2+e3-cost_constrain,0))^2'); %This is to compute Etot; this can be implemented as a control constraint as well, see bellow
...
for iexp=1:inputs.exps.n_exp
inputs.iocsol.n_steps{iexp}=10;               % [] Stimuli definition
...
inputs.model.beta=0;    % Paremeter value to handle regularization in PE (any real number)
                          % 0: means no regularization   
...
inputs.nlpsol.eSS.local.solver='solnp';  % Choose local solver
...
inputs.nlpsol.eSS.log_var = [1:3+3*inputs.iocsol.n_steps{1}];           % Decision variables to be considered in log-scale. Suitable for this problem with parameters                                           % ranging from 0 to 1.
inputs.nlpsol.eSS.maxeval = 2500000;
inputs.nlpsol.eSS.maxtime = 30;            % Extend duration for refined solutions

inputs.nlpsol.reopt='on';                            % Reoptimization after convergence
inputs.nlpsol.reopt_local_solver='fmincon';          % Solver for reoptimization
