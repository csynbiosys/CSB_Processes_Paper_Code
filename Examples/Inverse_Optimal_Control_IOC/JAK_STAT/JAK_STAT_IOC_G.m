...
% EpoR is assumed to follow a Gaussian - increase + smooth convex decrease
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
inputs.pathd.results_folder='JAK_STATG';      % Folder to keep re(u1+u2)lts (in Re(u1+u2)lts) for a given problem          
inputs.pathd.short_name='JSG';                % To identify figures and reports for a given problem   
inputs.pathd.runident='testG1';               % Run identifieer
...
inputs.model.stimulus_names=char('EpoA','Epot','EpoS');                

inputs.model.eqns=...                                      % Equations describing system dynamics. Time derivatives are regarded 'd'st_name''
char('pEpoR=EpoA*exp(-0.5*((t-Epot)^2)/EpoS^2)',...
'dSTAT =   p4 * nSTAT5 * (0.45/1.4)    - p1 * pEpoR * STAT',...
...
for iexp=1:inputs.exps.n_exp
 ...
 inputs.iocsol.u_min{iexp}=[0; 0.5; 0.5];
 inputs.iocsol.u_max{iexp}=[1; 30; 30];           % Minimum and maximum value for the input
 inputs.iocsol.u_guess{iexp}=[0.5; 15; 15];       % Guess value for the input
 inputs.iocsol.t_con{iexp}= [0 inputs.exps.t_f{iexp}];
end
...
inputs.model.alpha=0.0;   % Parameter value to handle control regularization (any real number)
...
inputs.nlpsol.eSS.log_var = [1:6];        % Decision variables to be considered in log-scale. Suitable for this problem with parameters
                                                     % ranging from 0 to 1.
inputs.nlpsol.eSS.maxeval = 1500000;
inputs.nlpsol.eSS.maxtime = 60;           % Maximum CPU time.
