...
inputs.model.par_names=char('c1','c2','c3','u2');                           % Names of the parameters                     
inputs.model.stimulus_names=char('u1');                    % Names of the stimuli, inputs or controls                      
...
% INITIAL CONDITIONS TO BE OPTIMISED
 ...
 inputs.DOsol.id_y0=char('y2');                                   % initial conditions to be designed
 inputs.DOsol.y0_max=[0.125];                                         % Maximum allowed values for the initial conditions
 inputs.DOsol.y0_min= [0 ];                                       % Minimum allowed values for the initial conditions
 inputs.DOsol.y0_guess= [0.1 ]; 
 

 % CONSTANT CONTROLS (FOR THE CASES IN WHICH CONSTANT & TIME VARYING ARE
 % PRESENT)

 inputs.DOsol.id_par=char('u2');                                   % parameters / sustained controls
 inputs.DOsol.par_max=[1];                                         % Maximum allowed values for the paramters/ sustained controls
 inputs.DOsol.par_min= [0.25 ];                                       % Minimum allowed values for the paramters/ sustained controls
 inputs.DOsol.par_guess= [0.5 ]; 
...
inputs.nlpsol.nsga2.maxGen  = 300;                        % nsga2 max number of generations
...
% WEIGHTED SUM METHOD
%inputs.nlpsol.nlpsolver='wsm_fmincon';                  % Weighted sum method with a local
