...
%         oid_y0 = [sim.sim.states{1}(end,:) 0];
    ...
    inputs.exps.t_con{iexp}=linspace((i-1)*duration,(i*duration),numSteps);            % Switching times
    inputs.exps.u_min{iexp}=0*ones(1,numSteps);    % Lower boundary for the input value
    inputs.exps.u_max{iexp}=1000*ones(1,numSteps); % Upper boundary for the input value
    ...
    if (numLoops~=1)
        ...
        oid_y0 = [results{i}.sim.states(end,:) 0];
    else
